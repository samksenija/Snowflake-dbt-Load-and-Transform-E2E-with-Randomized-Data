...
## Process
### Load & Extract
The process consists of storing all data sources in centralised environment. Once data sources are stored in dedicated internal stage, needed data is extracted from unstructured sources (PDFs in this case, sample of PDF can be found in `LT` folder (`transaction_REF51.pdf`)). To extract data Snowflake's `AI_EXTRACT` is used, which is feature that allows trough set of questions to extract needed data. This is  example how `AI_EXTRACT` was used in my code:
```
SELECT AI_EXTRACT(
